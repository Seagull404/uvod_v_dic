# dodajanje v slovarje
# slovar["ključ"] = vrednost
# {"ključ" : "vrednost", ključ: "vrednost 2"}

prazen={}
prazen["ključ"] = "vrednost"
print(prazen)
...
# varni dostop
# slovar.get("ključ", "vrnemo če ni ključa")
print(imenik.get("Ažbe", "Ni Ažbeta"))
print(imenik.get("neznanec"), 0)


# shrek scenari preštej posamezne črke
...
# gremo čez vse črke
# preverimo če je črka v slovarju
# če je ni jo dodamo in nastavimo na 1
# če pa je pa ji dodamo 1
scenarij = scenarij.upper().replace(".", "").replace(",", "").replace("!", "").replace("?", "").split()
crke = {}
for s in scenarij:
    if crke.get(s, False):
        crke[s] += 1
...
besede = {}
for i in scenarij:
    if besede.get(i, False):
        besede[i] += 1
    else:
        besede[i] = 1
print(besede)

# https://guthub.com/colaric/zapiskiRPS